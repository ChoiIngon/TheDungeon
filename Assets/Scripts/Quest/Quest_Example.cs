using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class Quest_Example : Quest {
	public Quest_Example()
	{
		/*
		this.id = "QUEST_EXAMPLE";
		this.name = "Example Quest";
		this.state = QuestData.State.Invalid;
		this.triggers.Add (new QuestTrigger_LessCompleteQuestCount (0));
		this.progresses.Add (new Progress ("Go into the dungeon", Progress.Type.CrrentLocation, "Dungeon", 1));
		start_dialogue = new Dialogue ();
		start_dialogue.speaker = "Far Seer";
		start_dialogue.scripts = new string[] {
			"Welcome to \'Last Village\'\nYour first quest is go to the dungeon and reach the last level\n",
			"Touch enter button"
		};
		*/
	}
}
